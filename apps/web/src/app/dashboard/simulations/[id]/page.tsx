...
export default function SimulationDetailPage({ params }: { params: { id: string } }): JSX.Element {
  ...
  useEffect(() => {
    async function load() {
      const res = await simulations.get(params.id);
      if (res.data) {
  ...
  const handleReplay = async (run: SimRunRow) => {
    ...
    const res = await simulations.replay(params.id);
    setTraceLoading(false);
    if (res.error) {
      if ((res.error as { upgrade_required?: boolean }).upgrade_required) {
